...
P0：作者明确承认且影响核心结论的 limitation 或失败现象；P1：多个论文重复出现、但缺乏统一解释的问题；P2：结果与直觉/假设不一致的反常现象；P3：方法依赖但尚未检验的关键假设或机制条件；P4：跨任务、规模或数据分布迁移时暴露的边界。P0–P4 是“先追踪什么”的优先级，不是正确性评分；每条线索保留证据出处和当前解释。
